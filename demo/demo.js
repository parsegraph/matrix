...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>matrix</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>matrix <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-matrix.js'>parsegraph-matrix.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See matrix build information at http://localhost:${port}`);
});
